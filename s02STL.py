...
# ========== 主函数：处理 'close' 列 ==========
def process_close_stl(filepath: Path):
    try:
        ...
        series = pd.to_numeric(daily_data["close"], errors="coerce").asfreq("D")

        # === 处理缺失值 ===
        series = forward_fill_with_limit(series, max_gap=5).dropna()
        ...
        # === 绘图 ===
        image_path = results_dir / "s02btc_close_stl_daily.png"
        ...
        ax_obs.set_title("Observed (Close Price)")
        ...
        plot_acf(resid, ax=ax_acf, lags=min(40, len(resid) - 1))
...
if __name__ == "__main__":
    result = process_close_stl(file_path)
    print("\n✅ All done!")